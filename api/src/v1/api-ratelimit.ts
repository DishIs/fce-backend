    ...
    [
      {
        $set: {
          apiCredits: {
            $cond: [
              { $gt: [{ $ifNull: ['$apiCredits', 0] }, 0] },
              { $subtract: [{ $ifNull: ['$apiCredits', 0] }, 1] },
              { $ifNull: ['$apiCredits', 0] }
            ]
          },
          proBonusCredits: {
            $cond: [
              {
                $and: [
                  { $lte: [{ $ifNull: ['$apiCredits', 0] }, 0] },
                  { $gt: [{ $ifNull: ['$proBonusCredits', 0] }, 0] }
                ]
              },
              { $subtract: [{ $ifNull: ['$proBonusCredits', 0] }, 1] },
              { $ifNull: ['$proBonusCredits', 0] }
            ]
          }
        }
      }
